...
DEFAULT_OUTPUT = PROJECT_DIR / "out_jan_div35_tbm.csv"
...
def add_variance_flags_octdec_vs_jan(
    ...
    """Variance flags: baseline 3 months avg vs report month; when tbm_col set, grain is (TBM, doctor)."""
    out = df.copy()
    for c in product_cols:
        if c in out.columns:
            out[c] = pd.to_numeric(out[c], errors="coerce").fillna(0)
            ...
            out[c] = 0
    out["_total_rx"] = out[product_cols].sum(axis=1)
    b0, b1, b2 = baseline_months[0], baseline_months[1], baseline_months[2]
    if tbm_col and tbm_col in out.columns:
        ...
        t0 = out.loc[out[monthperiod_col] == b0].groupby(group_keys)["_total_rx"].sum().reindex(all_index).fillna(0)
        t1 = out.loc[out[monthperiod_col] == b1].groupby(group_keys)["_total_rx"].sum().reindex(all_index).fillna(0)
        t2 = out.loc[out[monthperiod_col] == b2].groupby(group_keys)["_total_rx"].sum().reindex(all_index).fillna(0)
        report_totals = out.loc[out[monthperiod_col] == report_jan].groupby(group_keys)["_total_rx"].sum().reindex(all_index).fillna(0)
        avg_baseline = (t0 + t1 + t2) / 3.0
        mask_both_zero = (avg_baseline == 0) & (report_totals == 0)
        mask_baseline_zero_report_positive = (avg_baseline == 0) & (report_totals > 0)
        percent_diff = np.where(mask_both_zero, 0.0, np.where(mask_baseline_zero_report_positive, np.nan, ((report_totals - avg_baseline) / avg_baseline) * 100.0))
        variance_ser = pd.Series(np.where(mask_both_zero, False, np.where(mask_baseline_zero_report_positive, True, np.abs(percent_diff) > 25.0)), index=report_totals.index)
        increasing_ser = pd.Series(np.where(mask_both_zero, False, np.where(mask_baseline_zero_report_positive, True, percent_diff >= 25.0)), index=report_totals.index)
        decreasing_ser = pd.Series(np.where(mask_both_zero, False, np.where(mask_baseline_zero_report_positive, False, percent_diff <= -25.0)), index=report_totals.index)
        all_zero_ser = (report_totals == 0)
        sum_last_3_months = t0 + t1 + t2
        ...
        t0 = out.loc[out[monthperiod_col] == b0].groupby(doctor_col)["_total_rx"].sum().reindex(all_doctors).fillna(0)
        t1 = out.loc[out[monthperiod_col] == b1].groupby(doctor_col)["_total_rx"].sum().reindex(all_doctors).fillna(0)
        t2 = out.loc[out[monthperiod_col] == b2].groupby(doctor_col)["_total_rx"].sum().reindex(all_doctors).fillna(0)
